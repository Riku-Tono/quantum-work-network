...
### Signed-power integration

`integrate_signed_power` uses the trapezoidal rule for net energy. For
`energy_in` and `energy_out`, an interval whose endpoint powers have opposite
signs is split at the linearly interpolated zero crossing. For example,
`[(0.0, 1.0), (1.0, -1.0)]` yields `energy_net = 0`,
`energy_in = 0.25`, and `energy_out = 0.25`.
