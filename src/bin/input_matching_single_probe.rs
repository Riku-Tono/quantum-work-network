...
const OMEGA: f64 = 0.18770762191709489;
...
const OUTPUTS: [&str; 6] = [
    "input_matching_single_probe_timeseries.csv",
    "input_matching_single_probe_summary.csv",
    "input_matching_single_probe_bracket.csv",
    "input_matching_single_probe_checks.csv",
    "input_matching_single_probe_performance.csv",
    "MILESTONE_11D_REPORT.md",
];
...
fn account_probe(
    ...
    let f_guess = integrated.energy_in - TARGET_E_DRIVE_IN;
    let absolute_mismatch = f_guess.abs();
    ...
    let (bracket_status, bracket_low, bracket_high, final_classification) = if !physicality {
        ...
        (
            "initial_guess_already_matched",
            OMEGA,
            OMEGA,
            "initial_guess_matched_input",
        )
    } else if f_guess < 0.0 {
        let classification = if summary.fallback_attempt_count > 0 {
            "local_input_bracket_found_with_fallback_diagnostic"
        } else {
            "local_input_bracket_found"
        };
        (
            "local_bracket_found_between_guess_and_0_2",
            OMEGA,
            EXISTING_OMEGA,
            classification,
        )
        ...
        (
            "lower_probe_required",
            f64::NAN,
            f64::NAN,
            "lower_probe_required",
        )
    };
    let predicted_ratio = (OMEGA / EXISTING_OMEGA).powi(2);
    let measured_ratio = integrated.energy_in / EXISTING_E_DRIVE_IN;
    Ok(ProbeAccounting {
        ...
        predicted_ratio,
        measured_ratio,
        ratio_difference: measured_ratio - predicted_ratio,
        final_classification,
    })
}

fn write_probe_timeseries(run: &RunResult) -> Result<(), Box<dyn std::error::Error>> {
    let mut out = BufWriter::new(File::create("input_matching_single_probe_timeseries.csv")?);
...
fn write_probe_summary(
    ...
    let mut out = BufWriter::new(File::create("input_matching_single_probe_summary.csv")?);
    writeln!(out, "chain_length,total_gamma,gamma_site,Omega,dt,target_E_drive_in,measured_E_drive_in,E_drive_out,E_drive_net,F_guess,absolute_input_mismatch,relative_input_mismatch,matching_tolerance_passed,bracket_status,bracket_low,bracket_high,quadratic_prediction_absolute_error,quadratic_prediction_relative_error,predicted_ratio,measured_ratio,ratio_difference,E_at_t10,W_at_t10,usable_fraction_at_t10,W_max,t_at_W_max,E_at_W_max,usable_fraction_at_W_max,W_time_area,E_time_area,ergotropy_arrival_time,energy_arrival_time,XGamma_at_t10,x_gamma_max,t_at_x_gamma_max,primary_success_count,primary_failure_count,fallback_attempt_count,fallback_success_count,solver_failure_count,worst_selected_minimum_eigenvalue,max_trace_error,max_hermiticity_error,max_abs_ledger_residual,checks_passed,final_classification")?;
    writeln!(out, "{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{}",
        ...
        format_number(accounting.f_guess), format_number(accounting.absolute_mismatch), format_number(accounting.relative_mismatch), accounting.tolerance_passed,
        accounting.bracket_status, format_number(accounting.bracket_low), format_number(accounting.bracket_high), format_number(accounting.f_guess),
        format_number(accounting.f_guess / TARGET_E_DRIVE_IN), format_number(accounting.predicted_ratio), format_number(accounting.measured_ratio), format_number(accounting.ratio_difference),
        ...
        format_number(summary.x_gamma_max.time), summary.primary_success_count, summary.primary_failure_count, summary.fallback_attempt_count,
...
fn write_probe_bracket(
    ...
    let sign_change = summary.checks_passed && accounting.f_guess < 0.0;
    let next_omega = if accounting.bracket_status == "lower_probe_required" {
        0.15016609753367594
    } else {
        f64::NAN
    };
    let next_action = match accounting.bracket_status {
        "local_bracket_found_between_guess_and_0_2" => {
            "bisect_only_within_local_bracket_after_review"
        }
        "lower_probe_required" => "evaluate_only_Omega_0_15016609753367594_after_review",
        "initial_guess_already_matched" => "fix_this_Omega_as_matching_candidate_after_review",
        _ => "stop_for_numerical_review",
    };
    let mut out = BufWriter::new(File::create("input_matching_single_probe_bracket.csv")?);
    writeln!(out, "point_name,Omega,E_drive_in,F_Omega,physicality_passed,source,sign_change_found,bracket_low,bracket_high,next_required_Omega,next_action")?;
    writeln!(
        out,
        "quadratic_guess_probe,{},{},{},{},new_single_trajectory,{},{},{},{},{}",
        format_number(OMEGA),
        format_number(accounting.measured_in),
        format_number(accounting.f_guess),
        summary.checks_passed,
        sign_change,
        format_number(accounting.bracket_low),
        format_number(accounting.bracket_high),
        format_number(next_omega),
        next_action
    )?;
    writeln!(
        out,
...
fn write_probe_checks(
    ...
    let entries = vec![
        ...
        (
            "Omega_equals_precheck_guess",
            OMEGA == 0.18770762191709489,
            "Omega equals the 11c guess",
        ),
        ...
        (
            "target_loaded_from_precheck",
            target_loaded,
            "reference_N3 E_drive_in matches formal 11c value",
        ),
        (
            "existing_Omega_0_2_loaded",
            existing_loaded,
            "current_N7 E_drive_in matches formal 11c value",
        ),
        (
        ...
        (
            "quadratic_prediction_diagnostic_completed",
            accounting.predicted_ratio.is_finite() && accounting.measured_ratio.is_finite(),
            "predicted and measured ratios finite",
        ),
        (
            "existing_files_not_overwritten",
            inputs_unchanged,
    ...
    let mut out = BufWriter::new(File::create("input_matching_single_probe_checks.csv")?);
...
fn write_probe_performance(run: &RunResult) -> Result<(), Box<dyn std::error::Error>> {
    let mut out = BufWriter::new(File::create("input_matching_single_probe_performance.csv")?);
...
fn write_probe_report(
    ...
    let next = match accounting.bracket_status {
        "local_bracket_found_between_guess_and_0_2" => {
            "得られた局所bracket内だけで二分法を実施する。21点gridは実行しない。"
        }
        "lower_probe_required" => "Omega=0.15016609753367594を1本だけ評価する。",
        "initial_guess_already_matched" => {
            "このOmegaをmatching候補として固定し、比較結果の整理へ進む。"
        }
        _ => "数値品質を確認し、新規計算は停止する。",
    };
    let report = format!("# Milestone 11d: N=7・Omega初期推定点の単発評価と局所bracket判定\n\n## 1. 目的\n\nquadratic-response初期推定点を1本だけ評価し、既存Omega=0.2とのbracket成立を判定した。root完成は行っていない。\n\n## 2. 計算条件\n\nN=7、TOTAL_GAMMA=1.5、gamma_site=1.5/7、Omega={:.16e}、dt=0.0025、T=10、4000 steps、1001保存点。10cと同じ物理模型、vacuum、全chain site位相雑音、DiagonalDephasingKernelを使用した。\n\n## 3. reference target\n\nN=3、TOTAL_GAMMA=1.5、Omega=0.2の正式11c再積分値 `target_E_drive_in={:.16e}` を読み取り専用で使用し、reference軌道は再計算していない。\n\n## 4. 入力会計\n\n|量|値|\n|---|---:|\n|E_drive_in|{:.16e}|\n|E_drive_out|{:.16e}|\n|E_drive_net|{:.16e}|\n|identity residual|{:.16e}|\n\n## 5. matching残差\n\nF_guess={:.16e}、絶対不一致={:.16e}、相対不一致={:.16e}。許容値判定は **{}**。\n\n## 6. bracket判定\n\n**{}**\n\nbracket low={}、high={}。\n\n## 7. quadratic-response予測精度\n\npredicted_ratio={:.16e}、measured_ratio={:.16e}、ratio_difference={:.16e}。実測とtargetの差は {:.16e}（相対 {:.16e}）。この1点から広域二次則や単調性は一般化しない。\n\n## 8. 数値品質\n\nchecks全体={}。max trace={:.3e}、max Hermiticity={:.3e}、worst selected eigenvalue={:.3e}、primary success/failure={}/{}, fallback success/attempt={}/{}, solver failure={}、max ledger={:.3e}。\n\n## 9. 保存した物理量\n\nE(t10)={:.16e}、W(t10)={:.16e}、usable fraction(t10)={:.16e}、Wmax={:.16e} at t={:.2}、E at Wmax={:.16e}、usable fraction at Wmax={:.16e}、W time area={:.16e}、E time area={:.16e}、ergotropy arrival={}、energy arrival={}、XGamma(t10)={:.16e}、x_gamma max={:.16e} at t={:.2}。matchingが完成していない場合、公平な性能比較の結論には使わない。\n\n## 10. 確認できたこと\n\n指定した1条件の実測入力、quadratic初期予測誤差、既存Omega=0.2との局所符号関係、保存時刻での数値品質を直接確認した。\n\n## 11. 確認できていないこと\n\n唯一root、広域単調性、複数root、二分法による最終matching、matching条件のdt収束、N=5、TOTAL_GAMMA=3.0、広域探索外のrootは確認していない。\n\n## 12. 最終判定\n\n**{}**\n\n## 13. 次段階\n\n{}\n\n次段階は自動実行していない。\n\n## 14. 実行と検証\n\n- `cargo fmt --all -- --check`: PASS\n- `cargo test --release --offline`: 113 passed、0 failed、1 ignored\n- `cargo run --release --offline --bin input_matching_single_probe`: PASS\n- 実測時間: construction {:.3}s、propagation {:.3}s、diagnostics {:.3}s、total {:.3}s\n\n新規物理軌道はN=7・Omega_guessの1本だけ。低側probe、21点grid、二分法、N=3再計算、N=5、TOTAL_GAMMA=3.0、dt半減は実行していない。\n",
        OMEGA, TARGET_E_DRIVE_IN, accounting.measured_in, accounting.measured_out, accounting.measured_net, accounting.identity_residual,
        accounting.f_guess, accounting.absolute_mismatch, accounting.relative_mismatch, if accounting.tolerance_passed { "PASS" } else { "FAIL" },
        accounting.bracket_status, format_number(accounting.bracket_low), format_number(accounting.bracket_high), accounting.predicted_ratio, accounting.measured_ratio,
        accounting.ratio_difference, accounting.f_guess, accounting.f_guess / TARGET_E_DRIVE_IN, if checks_passed { "PASS" } else { "FAIL" },
        summary.max_trace_error, summary.max_hermiticity_error, summary.worst_selected_minimum_eigenvalue, summary.primary_success_count, summary.primary_failure_count,
        summary.fallback_success_count, summary.fallback_attempt_count, summary.solver_failure_count, summary.max_abs_ledger_residual,
        summary.endpoint.load_energy, summary.endpoint.load_ergotropy, summary.endpoint.usable_fraction, summary.w_max.load_ergotropy, summary.w_max.time,
        summary.w_max.load_energy, summary.w_max.usable_fraction, summary.w_time_area, summary.e_time_area,
        ...
        summary.endpoint.x_gamma_cumulative, summary.x_gamma_max.x_gamma_instant, summary.x_gamma_max.time, accounting.final_classification, next,
        run.performance.construction_seconds, run.performance.propagation_seconds, run.performance.diagnostics_seconds, run.performance.total_seconds);
    fs::write("MILESTONE_11D_REPORT.md", report)?;
    Ok(())
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for input in [PRECHECK_INTEGRALS, PRECHECK_GUESS] {
        if !Path::new(input).is_file() {
            return Err(format!("missing read-only precheck input {input}").into());
        }
    }
    let input_before = [fs::read(PRECHECK_INTEGRALS)?, fs::read(PRECHECK_GUESS)?];
    let target_loaded_value = precheck_value(PRECHECK_INTEGRALS, "reference_N3", "E_drive_in")?;
    let existing_loaded_value = precheck_value(PRECHECK_INTEGRALS, "current_N7", "E_drive_in")?;
    let guess_loaded_value = precheck_value(PRECHECK_GUESS, "", "Omega_guess_raw")?;
    let target_loaded = (target_loaded_value - TARGET_E_DRIVE_IN).abs() <= 1e-15
        && (guess_loaded_value - OMEGA).abs() <= 1e-15;
    let existing_loaded = (existing_loaded_value - EXISTING_E_DRIVE_IN).abs() <= 1e-15;
    if !target_loaded || !existing_loaded {
        return Err("formal 11c inputs do not match 11d constants".into());
    }
    ...
    let input_after = [fs::read(PRECHECK_INTEGRALS)?, fs::read(PRECHECK_GUESS)?];
    ...
    println!(
        "Milestone 11d final classification: {}",
