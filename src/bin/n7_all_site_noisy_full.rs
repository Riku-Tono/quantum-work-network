...
const GAMMAS: [f64; N] = [GAMMA_PHI; N];
...
const OUTPUTS: [&str; 11] = [
    "n7_all_site_noisy_timeseries.csv",
    "n7_all_site_noisy_site_populations.csv",
    "n7_all_site_noisy_summary.csv",
    "n7_all_site_noisy_arrivals.csv",
    "n7_all_site_noisy_windows.csv",
    "n7_noise_comparison.csv",
    "chain_length_noisy_comparison.csv",
    "chain_length_noise_loss.csv",
    "n7_all_site_noisy_checks.csv",
    "n7_all_site_noisy_performance.csv",
...
fn construction_checks(
    ...
    for row in 0..DIM {
        ...
        for col in 0..DIM {
            ...
            let col_chain = col / 3;
            let expected = ((row_chain ^ col_chain).count_ones() as f64) * GAMMA_PHI;
    ...
    let mut checks = vec![
        ...
        (
            ...
            GAMMAS
                ...
                .to_string(),
            "7".into(),
            GAMMAS.iter().filter(|gamma| **gamma > 0.0).count() == 7,
        ),
        (
            "all_site_gammas".into(),
            "0.5x7".into(),
            "0.5x7".into(),
            GAMMAS.iter().all(|gamma| *gamma == 0.5),
        ),
        (
...
fn run_full() -> Result<(RunResult, Vec<(String, String, String, bool)>), Box<dyn std::error::Error>>
{
    ...
    let checks = construction_checks(&ops, &rho, &kernel)?;
    ...
    let (power0, dephasing_power0) = instantaneous_powers(&rho, 0.0, &ops, &kernel)?;
    ...
    for step in 0..steps {
        ...
        rho = rk4_step(&rho, time, &ops, &kernel)?;
        ...
        if (step + 1) % SAVE_STEPS == 0 {
            ...
            let (current_power_complex, current_dephasing_power_complex) =
                instantaneous_powers(&rho, now, &ops, &kernel)?;
...
fn write_timeseries(rows: &[Row]) -> std::io::Result<()> {
    let mut w = BufWriter::new(File::create("n7_all_site_noisy_timeseries.csv")?);
    writeln!(w, "condition,chain_length,noise_condition,noisy_site_count,gamma_phi_per_noisy_site,hilbert_dimension,time,Omega,drive_envelope,load_energy,load_ergotropy,load_diagonal_ergotropy,load_coherence_ergotropy,load_coherence_l1,usable_fraction,drive_energy_in,drive_energy_net,dephasing_energy_net,W_over_Ein,total_chain_population,load_population_0,load_population_1,load_population_2,load_top_level_population,bare_network_energy,drive_power,dephasing_power,trace_error,hermiticity_error,min_eigenvalue,energy_ledger_residual")?;
    for r in rows {
        let values = [
            r.time,
            OMEGA,
        ...
        writeln!(
            w,
            "N7_all_site_noisy,7,all_site_noisy,7,0.5,384,{}",
...
fn write_sites(rows: &[Row]) -> std::io::Result<()> {
    let mut w = BufWriter::new(File::create("n7_all_site_noisy_site_populations.csv")?);
    writeln!(
        w,
...
fn write_arrivals(summary: &Summary) -> std::io::Result<()> {
    let mut w = BufWriter::new(File::create("n7_all_site_noisy_arrivals.csv")?);
...
fn write_summary(summary: &Summary) -> std::io::Result<()> {
    ...
    let mut out = BufWriter::new(File::create("n7_all_site_noisy_summary.csv")?);
    writeln!(out, "condition,chain_length,noise_condition,noisy_site_count,gamma_phi_per_noisy_site,E_at_t10,W_at_t10,usable_fraction_at_t10,coherence_L1_at_t10,drive_energy_in_at_t10,drive_energy_net_at_t10,dephasing_energy_net_at_t10,W_over_Ein_at_t10,E_max,t_at_E_max,coherence_L1_max,t_at_coherence_L1_max,W_max,t_at_W_max,E_at_W_max,usable_fraction_at_W_max,coherence_L1_at_W_max,drive_energy_in_at_W_max,dephasing_energy_net_at_W_max,W_over_Ein_at_W_max,energy_arrival_time,ergotropy_arrival_time,W_10_percent_arrival_time,W_50_percent_arrival_time,E_time_area_0_to_t10,W_time_area_0_to_t10,max_load_top_level_population,max_trace_error,max_hermiticity_error,minimum_density_eigenvalue,max_abs_energy_ledger_residual,maximum_drive_power_imaginary_part,maximum_dephasing_power_imaginary_part,finite_values_pass,endpoint_peak_classification")?;
    let values = [
        e.energy,
        e.work,
        e.usable,
    ...
    writeln!(
        out,
        "N7_all_site_noisy,7,all_site_noisy,7,0.5,{},{},{}",
        values.iter().map(|x| fmt(*x)).collect::<Vec<_>>().join(","),
        summary.all_finite,
        summary.peak_class
    )?;
...
fn write_windows(rows: &[Row]) -> std::io::Result<()> {
    let mut w = BufWriter::new(File::create("n7_all_site_noisy_windows.csv")?);
    writeln!(w, "condition,window_name,time_start,time_end,point_count,mean_load_energy,mean_load_ergotropy,mean_usable_fraction,mean_coherence_L1,E_time_area,W_time_area,mean_total_chain_population,mean_drive_power,mean_dephasing_power,drive_energy_net_change,dephasing_energy_net_change,maximum_load_ergotropy,time_of_window_max_W")?;
    for (name, start, end, include_start) in [
        ...
        let values = [
            ...
            subset.iter().map(|r| r.drive_power).sum::<f64>() / n,
            subset.iter().map(|r| r.dephasing_power).sum::<f64>() / n,
...
fn append_check(
    ...
    writeln!(
        w,
        "{stage},N7_all_site_noisy,{check},{observed},{expected},{}",
...
fn write_checks(
    ...
    let mut w = BufWriter::new(File::create("n7_all_site_noisy_checks.csv")?);
    ...
    let checks: Vec<(&str, String, String, bool)> = vec![
        ...
        ("noisy_site_count", N.to_string(), "7".into(), N == 7),
        (
            "all_site_gamma",
            "0.5x7".into(),
            "0.5x7".into(),
...
fn write_performance(run: &RunResult) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut w = BufWriter::new(File::create("n7_all_site_noisy_performance.csv")?);
    writeln!(w, "condition,chain_length,noisy_site_count,gamma_phi_per_noisy_site,hilbert_dimension,density_matrix_element_count,kernel_bytes,step_count,saved_time_points,construction_seconds,propagation_seconds,diagnostics_seconds,total_seconds,seconds_per_step,estimated_seconds_from_8c,measured_to_estimated_ratio,peak_memory_bytes_if_available,process_working_set_before,process_working_set_after,timeseries_rows,site_population_rows")?;
    let fields = vec![
        "N7_all_site_noisy".to_string(),
        "7".to_string(),
        "7".to_string(),
        "0.5".to_string(),
        "384".to_string(),
        "147456".to_string(),
        (DIM * DIM * std::mem::size_of::<f64>()).to_string(),
        "4000".to_string(),
