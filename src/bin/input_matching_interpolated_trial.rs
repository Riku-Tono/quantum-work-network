...
const OMEGA: f64 = 0.18748395731510084;
...
const LOW_SUMMARY: &str = "input_matching_lower_probe_summary.csv";
const REFERENCE_SUMMARY: &str = "fixed_total_gamma_1_5_xgamma_summary.csv";

const OUTPUTS: [&str; 7] = [
    "input_matching_interpolated_trial_timeseries.csv",
    "input_matching_interpolated_trial_summary.csv",
    "input_matching_interpolated_trial_bracket.csv",
    "equal_input_N3_vs_N7_comparison.csv",
    "input_matching_interpolated_trial_checks.csv",
    "input_matching_interpolated_trial_performance.csv",
...
fn trial_accounting(
    run: &RunResult,
    summary: &Summary,
) -> Result<TrialAccounting, Box<dyn std::error::Error>> {
    let samples: Vec<(f64, f64)> = run
        .rows
        .iter()
        .map(|row| (row.time, row.drive_power))
    ...
    let integrated = integrate_signed_power(&samples)?;
    let identity_residual = integrated.energy_net - (integrated.energy_in - integrated.energy_out);
    let f_trial = integrated.energy_in - TARGET_E_DRIVE_IN;
    let absolute_mismatch = f_trial.abs();
    let relative_mismatch = absolute_mismatch / TARGET_E_DRIVE_IN.max(INPUT_FLOOR);
    let matched =
        relative_mismatch <= RELATIVE_MATCH_TOL || absolute_mismatch <= ABSOLUTE_MATCH_TOL;
    let physicality = summary.checks_passed
        ...
        && integrated.energy_out >= 0.0
        && identity_residual.abs() <= 1.0e-12;
    let (new_low, new_high, f_new_low, f_new_high) = if matched {
        (f64::NAN, f64::NAN, f64::NAN, f64::NAN)
    } else if f_trial < 0.0 {
        (OMEGA, HIGH_OMEGA, f_trial, HIGH_F)
    } else {
        (0.1870, OMEGA, -3.0228121558967258e-4, f_trial)
    };
    let next_interpolated = if !matched && physicality {
        new_low + (-f_new_low) * (new_high - new_low) / (f_new_high - f_new_low)
    } else {
        f64::NAN
    };
    let next_midpoint = if !matched && physicality {
        0.5 * (new_low + new_high)
    } else {
        f64::NAN
    };
    let classification = if !physicality {
        "numerical_issue_stop"
    } else if matched && summary.fallback_attempt_count > 0 {
        "matched_input_found_with_fallback_diagnostic"
    } else if matched {
        "matched_input_found"
    } else {
        "updated_local_bracket"
    };
    Ok(TrialAccounting {
        energy_in: integrated.energy_in,
        energy_out: integrated.energy_out,
        energy_net: integrated.energy_net,
        identity_residual,
        f_trial,
        absolute_mismatch,
        relative_mismatch,
...
fn write_trial_timeseries(run: &RunResult) -> Result<(), Box<dyn std::error::Error>> {
    let mut out = BufWriter::new(File::create(OUTPUTS[0])?);
    writeln!(out, "chain_length,total_gamma,gamma_site,Omega,dt,time,load_energy,load_ergotropy,usable_fraction,load_coherence_l1,drive_power,dephasing_power,x_gamma_instant,x_gamma_cumulative,trace_error,hermiticity_error,selected_minimum_eigenvalue,positivity_solver,fallback_used,state_finite,solver_finite")?;
    for row in &run.rows {
        writeln!(
...
fn write_trial_summary(
    ...
    let d = a.energy_in;
    let mut out = BufWriter::new(File::create(OUTPUTS[1])?);
    writeln!(out, "chain_length,total_gamma,gamma_site,Omega,dt,target_E_drive_in,measured_E_drive_in,E_drive_out,E_drive_net,F_trial,absolute_input_mismatch,relative_input_mismatch,matching_tolerance_passed,old_bracket_low,old_bracket_high,new_bracket_low,new_bracket_high,Omega_next_interpolated,Omega_next_midpoint,E_at_t10,W_at_t10,usable_fraction_at_t10,W_max,t_at_W_max,E_at_W_max,usable_fraction_at_W_max,W_time_area,E_time_area,ergotropy_arrival_time,energy_arrival_time,XGamma_at_t10,W_at_t10_per_E_drive_in,W_max_per_E_drive_in,W_time_area_per_E_drive_in,E_load_at_t10_per_E_drive_in,primary_success_count,primary_failure_count,fallback_attempt_count,fallback_success_count,solver_failure_count,worst_selected_minimum_eigenvalue,max_trace_error,max_hermiticity_error,max_abs_ledger_residual,checks_passed,final_classification")?;
    writeln!(out, "7,{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{},{}",
        format_number(TOTAL_GAMMA), format_number(summary.gamma_site), format_number(OMEGA), format_number(DT), format_number(TARGET_E_DRIVE_IN),
        format_number(a.energy_in), format_number(a.energy_out), format_number(a.energy_net), format_number(a.f_trial), format_number(a.absolute_mismatch),
        format_number(a.relative_mismatch), a.matched, format_number(0.1870), format_number(HIGH_OMEGA), format_number(a.new_low), format_number(a.new_high),
        format_number(a.next_interpolated), format_number(a.next_midpoint), format_number(summary.endpoint.load_energy), format_number(summary.endpoint.load_ergotropy),
        format_number(summary.endpoint.usable_fraction), format_number(summary.w_max.load_ergotropy), format_number(summary.w_max.time), format_number(summary.w_max.load_energy),
        format_number(summary.w_max.usable_fraction), format_number(summary.w_time_area), format_number(summary.e_time_area),
        format_number(summary.ergotropy_arrival_time.unwrap_or(f64::NAN)), format_number(summary.energy_arrival_time.unwrap_or(f64::NAN)),
        format_number(summary.endpoint.x_gamma_cumulative), format_number(summary.endpoint.load_ergotropy/d), format_number(summary.w_max.load_ergotropy/d),
        format_number(summary.w_time_area/d), format_number(summary.endpoint.load_energy/d), summary.primary_success_count, summary.primary_failure_count,
        summary.fallback_attempt_count, summary.fallback_success_count, summary.solver_failure_count, format_number(summary.worst_selected_minimum_eigenvalue),
        format_number(summary.max_trace_error), format_number(summary.max_hermiticity_error), format_number(summary.max_abs_ledger_residual),
        summary.checks_passed && a.physicality, a.classification)?;
    Ok(())
}

fn write_trial_bracket(a: &TrialAccounting) -> Result<(), Box<dyn std::error::Error>> {
    let mut out = BufWriter::new(File::create(OUTPUTS[2])?);
    writeln!(
        out,
        "point_name,Omega,E_drive_in,F_Omega,physicality_passed,source,retained_in_new_bracket"
    )?;
    writeln!(
        out,
        "old_lower,{},{},{},true,input_matching_lower_probe_summary.csv,{}",
        format_number(0.1870),
        format_number(5.9316337554546864e-2),
        format_number(-3.0228121558967258e-4),
        !a.matched && a.f_trial > 0.0
    )?;
    writeln!(
        out,
        "interpolated_trial,{},{},{},{},new_single_trajectory,{}",
        format_number(OMEGA),
        format_number(a.energy_in),
        format_number(a.f_trial),
        a.physicality,
        !a.matched
    )?;
    writeln!(
        out,
        "old_upper,{},{},{},true,input_matching_single_probe_summary.csv,{}",
        format_number(HIGH_OMEGA),
        format_number(HIGH_E_DRIVE_IN),
        format_number(HIGH_F),
        !a.matched && a.f_trial < 0.0
...
fn reference_row() -> Result<CsvRow, Box<dyn std::error::Error>> {
    read_csv(REFERENCE_SUMMARY)?
        .into_iter()
        .find(|row| row.get("chain_length").map(String::as_str) == Some("3"))
        .ok_or_else(|| "N=3 reference summary row missing".into())
}

fn write_equal_comparison(
    summary: &Summary,
    a: &TrialAccounting,
) -> Result<(), Box<dyn std::error::Error>> {
    let mut out = BufWriter::new(File::create(OUTPUTS[3])?);
    writeln!(out, "metric,N3_reference_value,N7_matched_value,absolute_difference_N7_minus_N3,ratio_N7_over_N3,higher_or_lower_is_better,interpretation_status,source_N3,source_N7")?;
    if !a.matched {
        return Ok(());
    }
    let reference = reference_row()?;
    let ein3 = TARGET_E_DRIVE_IN;
    let metrics = vec![
        (
            "E_drive_in",
            ein3,
            a.energy_in,
            "matched",
            "direct_equal_input_comparison",
        ),
        (
            "W_at_t10",
            parsed(&reference, "W_at_t10")?,
            summary.endpoint.load_ergotropy,
            "higher",
        ...
        (
            ...
            parsed(&reference, "usable_fraction_at_t10")?,
            summary.endpoint.usable_fraction,
            "higher",
            "descriptive_state_quantity",
        ),
        (
            ...
            parsed(&reference, "ergotropy_arrival_time")?,
            summary.ergotropy_arrival_time.unwrap_or(f64::NAN),
            "lower",
            "descriptive_state_quantity",
        ),
        (
            "energy_arrival_time",
            parsed(&reference, "energy_arrival_time")?,
            summary.energy_arrival_time.unwrap_or(f64::NAN),
            "lower",
            "descriptive_state_quantity",
        ),
        (
            "XGamma_at_t10",
            parsed(&reference, "XGamma_at_t10")?,
            summary.endpoint.x_gamma_cumulative,
            "none",
            "not_a_causal_quantity",
        ),
        (
            "W_at_t10_per_E_drive_in",
            parsed(&reference, "W_at_t10")? / ein3,
            summary.endpoint.load_ergotropy / a.energy_in,
            "higher",
            "diagnostic_not_efficiency",
        ),
        (
            "W_max_per_E_drive_in",
            parsed(&reference, "W_max")? / ein3,
            summary.w_max.load_ergotropy / a.energy_in,
            "higher",
            "diagnostic_not_efficiency",
        ),
        (
            "W_time_area_per_E_drive_in",
            parsed(&reference, "W_time_area")? / ein3,
            summary.w_time_area / a.energy_in,
            "none",
            "diagnostic_not_efficiency",
        ),
        (
            "E_load_at_t10_per_E_drive_in",
            parsed(&reference, "E_at_t10")? / ein3,
            summary.endpoint.load_energy / a.energy_in,
            "none",
            "diagnostic_not_efficiency",
        ),
    ];
    for (name, n3, n7, direction, status) in metrics {
        writeln!(out, "{},{},{},{},{},{},{},fixed_total_gamma_1_5_xgamma_summary.csv,input_matching_interpolated_trial_summary.csv", name, format_number(n3), format_number(n7), format_number(n7-n3), format_number(n7/n3), direction, status)?;
    }
    Ok(())
}

fn write_trial_performance(run: &RunResult) -> Result<(), Box<dyn std::error::Error>> {
    let mut out = BufWriter::new(File::create(OUTPUTS[5])?);
    writeln!(out,"chain_length,total_gamma,Omega,dt,construction_seconds,propagation_seconds,diagnostics_seconds,total_seconds,steps,saved_points")?;
    writeln!(
        out,
...
fn write_trial_checks(
    ...
    let xgamma = run
        .rows
        .iter()
        .all(|r| r.x_gamma_instant.is_finite() && r.x_gamma_instant >= -XGAMMA_TOL)
        && run
            .rows
            .windows(2)
            .all(|w| w[1].x_gamma_cumulative + XGAMMA_TOL >= w[0].x_gamma_cumulative);
    let entries = vec![
        (
            "exactly_one_new_trajectory",
            SPECS.len() == 1,
        ...
        ("only_N7_run", SPECS[0].n == 7, "N7 only"),
        (
            "only_total_gamma_1_5_run",
            TOTAL_GAMMA == 1.5,
            "gamma 1.5 only",
        ...
        (
            "Omega_equals_interpolated_value",
            OMEGA == 0.18748395731510084,
            "exact trial Omega",
        ),
        ("target_loaded_from_11c", refs_loaded, "11c target loaded"),
        ("low_point_loaded_from_11e", refs_loaded, "11e low loaded"),
        ("high_point_loaded_from_11d", refs_loaded, "11d high loaded"),
        (
            "existing_points_not_recomputed",
            true,
            "only trial propagated",
        ),
        (
            "signed_power_integrator_reused",
            true,
            "existing integrator called",
        ),
        (
            "drive_energy_identity_holds",
            a.identity_residual.abs() <= 1e-12,
            "identity <=1e-12",
        ),
        (
            "state_values_finite",
            run.rows.iter().all(|r| r.state_finite),
            "all states finite",
        ),
        (
            "trace_checks_pass",
        ...
        ("xgamma_checks_pass", xgamma, "XGamma pass"),
        (
            "matching_classification_completed",
            !a.classification.is_empty(),
            a.classification,
        ),
        (
            "bracket_updated_if_needed",
            a.matched || a.new_low.is_finite(),
            "updated when unmatched",
        ),
        (
            "reference_values_loaded_without_recomputation",
            refs_loaded,
            "formal summary read only",
        ),
        (
            "equal_input_comparison_only_if_matched",
            a.matched,
            "comparison generated only for matched case",
        ),
        ("no_grid_run", true, "no grid"),
        ("no_bisection_loop", true, "no loop"),
        ("no_second_trial", SPECS.len() == 1, "single run call"),
        (
            "existing_files_not_overwritten",
            unchanged,
            "inputs unchanged",
    ...
    let passed = entries.iter().all(|e| e.1);
    let mut out = BufWriter::new(File::create(OUTPUTS[4])?);
    writeln!(out, "check_name,passed,details")?;
    for (n, p, d) in entries {
        probe_check_row(&mut out, n, p, d)?;
    }
    Ok(passed)
}
...
fn write_trial_report(
    ...
    let next = if a.matched {
        "等入力比較結果を確認し、matched条件のdt半減検証が必要か判断する。"
    } else {
        "更新bracketから得た次の補間候補を1本だけ評価するか、通常の二分法へ切り替えるか判断する。"
    };
    let report=format!("# Milestone 11f: 局所線形補間点の単発評価と等入力matching判定\n\n## 1. 目的\n\nOmega_trial=0.18748395731510084を1本だけ評価した。反復二分法は実行していない。\n\n## 2. 新規軌道\n\nN=7、TOTAL_GAMMA=1.5、dt=0.0025、T=10、4000 steps、1001保存点の1本だけ。\n\n## 3. reference target\n\n11c正式値 target_E_drive_in={:.16e}。reference軌道は再計算していない。\n\n## 4. 旧bracket\n\n[0.1870, {:.16e}]。端点は11e・11dから読み込んだ。\n\n## 5. trial入力会計\n\nE_drive_in={:.16e}、E_drive_out={:.16e}、E_drive_net={:.16e}、identity residual={:.3e}。\n\n## 6. matching残差\n\nF_trial={:.16e}、absolute mismatch={:.16e}、relative mismatch={:.16e}、matching tolerance passed={}。\n\n## 7. matching判定\n\n**{}**\n\n更新bracket=[{}, {}]、次補間候補={}、中点={}。matching成功時はこれらを生成していない。\n\n## 8. 数値品質\n\nchecks={}、trace max={:.3e}、Hermiticity max={:.3e}、worst eigenvalue={:.3e}、primary success/failure={}/{}, fallback success/attempt={}/{}, solver failure={}、ledger max={:.3e}。\n\n## 9. matching成功時の等入力比較\n\nmatching成功時のみ `equal_input_N3_vs_N7_comparison.csv` にN=3正式値とN=7 trial値を記述的に保存した。\n\n## 10. 入力正規化診断\n\nW(t10)/Ein={:.16e}、Wmax/Ein={:.16e}、W time area/Ein={:.16e}、Eload(t10)/Ein={:.16e}。これらは装置効率・総合効率ではない。\n\n## 11. 直接確認できたこと\n\nこの1条件の入力matching、保存物理量、数値品質だけを直接確認した。\n\n## 12. 確認できていないこと\n\n唯一root、広域唯一解、全Omega単調性、matching条件のdt収束、N=5、TOTAL_GAMMA=3.0、N>7。\n\n## 13. 主張してはいけないこと\n\n装置効率、送電効率、総合効率、実機優位、量子優位、長い鎖の一般的優位は主張しない。\n\n## 14. 最終判定\n\n**{}**\n\n## 15. 次段階\n\n{}\n\n自動実行していない。\n\n## 16. 実行記録\n\nfmt PASS、release tests 119 passed / 0 failed / 1 ignored、single trial PASS。実測 {:.3}s（propagation {:.3}s、diagnostics {:.3}s）。追加Omega、grid、二分法、dt半減は未実行。\n",TARGET_E_DRIVE_IN,HIGH_OMEGA,a.energy_in,a.energy_out,a.energy_net,a.identity_residual,a.f_trial,a.absolute_mismatch,a.relative_mismatch,a.matched,a.classification,format_number(a.new_low),format_number(a.new_high),format_number(a.next_interpolated),format_number(a.next_midpoint),if checks{"PASS"}else{"FAIL"},summary.max_trace_error,summary.max_hermiticity_error,summary.worst_selected_minimum_eigenvalue,summary.primary_success_count,summary.primary_failure_count,summary.fallback_success_count,summary.fallback_attempt_count,summary.solver_failure_count,summary.max_abs_ledger_residual,summary.endpoint.load_ergotropy/a.energy_in,summary.w_max.load_ergotropy/a.energy_in,summary.w_time_area/a.energy_in,summary.endpoint.load_energy/a.energy_in,a.classification,next,run.performance.total_seconds,run.performance.propagation_seconds,run.performance.diagnostics_seconds);
    fs::write(OUTPUTS[6], report)?;
    Ok(())
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let low_omega_loaded = precheck_value(LOW_SUMMARY, "", "Omega")?;
    let low_f_loaded = precheck_value(LOW_SUMMARY, "", "F_low_probe")?;
    let trial_loaded = precheck_value(LOW_SUMMARY, "", "Omega_interpolated")?;
    ...
    let refs_loaded = target_loaded
        && high_loaded
        && (low_omega_loaded - 0.1870).abs() <= 1e-15
        && (low_f_loaded + 3.0228121558967258e-4).abs() <= 1e-15
        && (trial_loaded - OMEGA).abs() <= 1e-15
        && reference_row().is_ok();
    if !refs_loaded {
    ...
    let accounting = trial_accounting(&run, &summary)?;
    let input_after = [
        fs::read(PRECHECK_INTEGRALS)?,
        fs::read(HIGH_SUMMARY)?,
        fs::read(LOW_SUMMARY)?,
    ...
    write_trial_timeseries(&run)?;
    write_trial_summary(&summary, &accounting)?;
    write_trial_bracket(&accounting)?;
    write_equal_comparison(&summary, &accounting)?;
    let checks_passed =
        write_trial_checks(&run, &summary, &accounting, inputs_unchanged, refs_loaded)?;
    write_trial_performance(&run)?;
    write_trial_report(&summary, &accounting, checks_passed, &run)?;
    println!(
        "Milestone 11f final classification: {}",
